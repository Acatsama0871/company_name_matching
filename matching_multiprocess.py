...
def extract_func(cur_focus_txt: str, cur_txt: str, search_df: pd.DataFrame, first_n_matches:int) -> Tuple[str, List[str], List[str], List[float]]:
    """
    ...
    Returns:
        Tuple[str, List[str], List[str], List[float]]: focus_txt, matched_names, matched_ids, matched_scores
    """
    ...
    return cur_focus_txt, cur_df.iloc[:first_n_matches]['Full Name'].tolist(), cur_df.iloc[:first_n_matches]['No. '].tolist(), cur_df.iloc[:first_n_matches]['final_score'].tolist()
...
@dask.delayed
def process_one(cur_path: str, match_based_on_first_n:int, first_n_matches:int) -> None:
    ...
    cur_target_df['cur_focus_target'] = cur_target_df['preprocessed'].apply(keep_fist_n, n=match_based_on_first_n)
    # apply function
    cur_focus_txt_list = []
    ...
    for _, cur_row in cur_target_df.iterrows():
        cur_focus_txt, cur_matched_names, cur_matched_nos, cur_matched_scores = extract_func(cur_row['cur_focus_target'], cur_row['preprocessed'], search_df, first_n_matches)
        cur_focus_txt_list.append(cur_focus_txt)
        for i, cur_temp in enumerate(zip(cur_matched_names, cur_matched_nos, cur_matched_scores)):
            cur_name = cur_temp[0]
            cur_no = cur_temp[1]
...
if __name__ == '__main__':
    ...
    delayed_objects = [process_one(cur_path, match_based_on_first_n=match_base_on_first_n_words, first_n_matches=return_first_n_matches) for cur_path in path_list]
